...
import { FileText, Download, Image as ImageIcon, Copy, Check, Bookmark, ChevronDown, ChevronUp, Pencil, X, RotateCcw } from 'lucide-react';
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm';
import rehypeSanitize, { defaultSchema } from 'rehype-sanitize';
import { usePromptsStore } from '../store/usePromptsStore';
import { getCostColorClass } from '../utils/costColors';

const COLLAPSE_THRESHOLD = 100; // Characters before collapse button appears
...
// Text Skeleton Loader Component (for chat models)
const TextSkeletonLoader = () => (
    <div className="flex justify-start" aria-label="Thinking...">
        <div
            data-testid="skeleton-loader"
            className="px-4 py-3 rounded-2xl rounded-tl-none ml-2 shadow-sm min-w-[200px] border border-brand-border overflow-hidden"
            style={{ background: 'oklch(0.18 0.025 264)' }}
        >
            <span className="sr-only">Thinking...</span>
            <div className="space-y-2.5">
                <div data-testid="skeleton-bar" className="h-3 rounded-full skeleton-shimmer" style={{ width: '85%' }}></div>
                <div data-testid="skeleton-bar" className="h-3 rounded-full skeleton-shimmer" style={{ width: '60%', animationDelay: '0.1s' }}></div>
                <div data-testid="skeleton-bar" className="h-3 rounded-full skeleton-shimmer" style={{ width: '75%', animationDelay: '0.2s' }}></div>
                <div data-testid="skeleton-bar" className="h-3 rounded-full skeleton-shimmer" style={{ width: '45%', animationDelay: '0.3s' }}></div>
            </div>
        </div>
    </div>
);

// Image Skeleton Loader Component (for image generation models)
const ImageSkeletonLoader = () => (
    <div className="flex justify-start" aria-label="Generating image...">
        <div
            data-testid="skeleton-loader"
            className="rounded-2xl rounded-tl-none ml-2 shadow-sm border border-brand-border overflow-hidden"
            style={{ background: 'oklch(0.18 0.025 264)' }}
        >
            <span className="sr-only">Generating image...</span>
            <div className="w-48 h-48 relative flex items-center justify-center">
                {/* Placeholder image icon with pulse */}
                <div className="skeleton-shimmer rounded-lg w-40 h-40 flex items-center justify-center">
...
const SkeletonLoader = ({ isImageModel = false }) => (
    isImageModel ? <ImageSkeletonLoader /> : <TextSkeletonLoader />
);

export default function MessageList({ messages, isLoading, isImageModel = false, onViewFile, onEditMessage }) {
    ...
    return (
        <div ref={containerRef} className="flex-1 overflow-y-auto p-4 space-y-5">
            {messages.map((m, i) => {
                const isCollapsed = collapsedMessages[i];
                const isLongMessage = m.content && m.content.length > COLLAPSE_THRESHOLD;
                ...
                return (
                    <div key={i} className={`group flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
                        ...
                        {/* Copy, Save, Edit Buttons for User (Left side) */}
                        {m.role === 'user' && (
                        ...
                        <div
                            className={`max-w-[88%] p-3.5 rounded-2xl text-[15px] leading-relaxed shadow-sm transition-all duration-200 ${m.role === 'user'
                            ...
                            {isEditing ? (
                                <div className="space-y-2">
                                    <textarea
                                        ...
                                        className="w-full p-2 bg-brand-input text-white border border-brand-border rounded-lg resize-none focus:outline-none focus:border-brand-cyan/50"
                                        rows={4}
                                        autoFocus
                                    />
                                    <div className="flex justify-end gap-2">
                                        ...
                                        <button
                                            ...
                                            className="p-1.5 bg-brand-cyan/20 hover:bg-brand-cyan/30 rounded-md text-brand-cyan"
                                ...
                                <>
                                    ...
                                    <div className="prose prose-invert prose-sm max-w-none break-words prose-p:leading-relaxed prose-pre:bg-black/40 prose-pre:border prose-pre:border-brand-border">
                                        ...
                                        >
                                            {isCollapsed ? m.content.slice(0, COLLAPSE_THRESHOLD) + '...' : m.content}
                                        </ReactMarkdown>
                                    </div>
                                    ...
                                    {m.role === 'assistant' && m.metadata && (
                                        <div className="mt-3 pt-2.5 border-t border-brand-border/40 flex items-center gap-3 text-[10px] text-gray-500 font-mono opacity-80">
                                            ...
                                            {m.metadata.cost != null && m.metadata.cost > 0 && (
                                                <span className={`flex items-center gap-0.5 ${getCostColorClass(m.metadata.cost)}`}>
                        ...
                        {/* Copy/Collapse Buttons for Assistant (Right side) */}
                        {m.role === 'assistant' && (
                            <div className="opacity-0 group-hover:opacity-100 transition-opacity self-center ml-2 shrink-0 flex flex-col gap-1">
                                <CopyButton text={m.content} label="Copy message" />
                                {isLongMessage && (
                                    <button
                                        onClick={() => toggleCollapse(i)}
            ...
            {isLoading && <SkeletonLoader isImageModel={isImageModel} />}
